...
#define PAN_PMODE_CHECK_SUP_TIME 1500
...
#define ANTI_TOUCH_RATIO 20
#define TOTAL_TOUCH_CNT 60 // Touch + anti-touch
#else
...
static int mAnti_touch_cnt=0;
#define ANTI_TOUCH_LIMIT_CNT  100
#define ANTI_TOUCH_LIMIT_CNT_AFTER  200
...
static void pan_check_anti_touch(uint8_t *quantum_msg)
{
	...
	dbg_touch("Touch cnt -> %d, Anti Touch cnt-> %d, INTTHR -> %d, NUMRPTTCH -> %d, SCRSTATUS -> %d, anti_touch_ratio -> %d, mAnti_touch_cnt -> %d\n",
			debugInfo.tch_ch,debugInfo.atch_ch, debugInfo.inntch_ch, debugInfo.numrpt_ch, debugInfo.scr_status, anti_touch_ratio,mAnti_touch_cnt);	

	if(pan_pmode_resume_cal && (mAnti_touch_cnt > ANTI_TOUCH_LIMIT_CNT)){
		dbg_cr("pan_check_anti_touch Anti_touch_calibration. mAnti_touch_cnt=> %d\n",mAnti_touch_cnt);
		calibrate_chip();
		mAnti_touch_cnt=0;
		return;
	}else if(!pan_pmode_resume_cal && (mAnti_touch_cnt > ANTI_TOUCH_LIMIT_CNT_AFTER)){
		dbg_cr("pan_check_anti_touch Anti_touch_calibration. mAnti_touch_cnt=> %d\n",mAnti_touch_cnt);
		calibrate_chip();
		mAnti_touch_cnt=0;
		return;    
