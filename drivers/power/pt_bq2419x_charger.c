...
#if defined(FEATURE_CHG_TERM_CTL)
...
static int bq2419x_iinlim_set(struct pt_bq2419x_chg_chip *chip, unsigned int uA)
{
	...
	/* to prevent entering HIZ state
		iinlim 100mA -> iinlim 500mA
	*/
	if (mA < bq2419x_iinlim_val[/*IINLIM_100MA*/IINLIM_500MA])
		mA = bq2419x_iinlim_val[/*IINLIM_100MA*/IINLIM_500MA];
...
static int can_start_recharging(struct pt_bq2419x_chg_chip *chip, u8 sys_stat)
{
	...
	if (evdata && evdata->data) {
		if (event == FB_EVENT_BLANK) {
			...
			if (*blank == FB_BLANK_UNBLANK)
				bq2419x_chg_term_en(chip, false);
			else if (*blank == FB_BLANK_POWERDOWN)
				bq2419x_chg_term_en(chip, true);
		}
	}
...
static void bq2419x_hw_init(struct pt_bq2419x_chg_chip *chip)
{
	...
	/*
		defualt USB current limit : 500mA
	 	- to prevent entering HIZ state
	 */
	gpio_set_value(chip->otg_gpio, 1);
...
update_heartbeat_work(struct work_struct *work)
{
	...
	if (get_prop_batt_present(chip)) {
		...
		sys_stat = bq2419x_get_sys_status(chip);
		if (battery_temp_trip_changed(chip, chip->batt_temp) || can_start_recharging(chip, sys_stat))
			set_appropriate_battery_current(chip);
